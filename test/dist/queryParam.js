...
describe("Check if getQuestParams gets correct params", function () {
    it("Should return string with username", function () {
        var req = { user: "test" };
        chai.expect(helpers_1.getQueryParams(req)).to.eql({ username: "test" });
    });
    it("Return object with undefined username", function () {
        var req = { test: "test", test2: "usergood" };
        chai.expect(helpers_1.getQueryParams(req)).to.eql({ username: undefined });
    });
});
